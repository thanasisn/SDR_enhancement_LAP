...
RES="$(parallel-ssh -i -h "$HOME/.pssh_host_files" ps u -C "$mcomand" )"
...
paste <( echo "$RES" | grep "\[SUCCESS\]" | cut -d' ' -f4 )                         \
      <( echo "$RES" | grep "$mcomand"  | sed 's/\/usr.*//g' | sed 's/athan//g' | sed 's/\/home\/condor\/executables\/libRadtran-2.0-64\/bin\///g' ) | sort -n -k 10
