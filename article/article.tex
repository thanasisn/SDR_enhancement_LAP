...
The use of the actual modelled values of clear sky GHI, described above,
can not provide us with a robust method to distinguish the CE cases, due
to the limited accuracy of the input data, and the natural variability
of the atmosphere. Our main focus was to positively identify over
irradiance events related to CEs. For this reason, we created an upper
envelope of the clear sky irradiance, above which, any GHI value can
safely be attributed to CE a event.

We had to select both, an appropriate set of atmospheric
parameterization, in Libradtran, and the thresholds of CE
identification. First we evaluated the performance of the modelled clear
radiation, for each of atmospheric level of clearness we have created as
...
\begin{equation}
\text{CE} : \text{E}_\text{i} > 15 + 1.04 \cdot \text{E}_\text{CSm,i} \,\,[W/m^2] \label{eq:CE4}
\end{equation} where: \(\theta\) is the solar zenith angle, \(\text{E}\)
the measured irradiance, \(\text{E}_\text{CSm}\) the selected modelled
