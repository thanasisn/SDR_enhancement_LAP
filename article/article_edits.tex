...
%DIF DEL /home/athan/MANUSCRIPTS/02_enhancement/SUBMISSION_01/article/article.tex   Wed Jul 10 10:31:02 2024
%DIF ADD /home/athan/MANUSCRIPTS/02_enhancement/article/article.tex                 Thu Sep  5 12:37:29 2024
...
\hypersetup{breaklinks=true,
            ...
            pdfauthor={},
%DIF 47c47
%DIF <             pdftitle={Long-term cloud enhancement events of global solar irradiance over Thessaloniki, Greece},
%DIF -------
            pdftitle={Analysis of cloud enhancement events in a 30-year record of global solar irradiance at Thessaloniki, Greece}, %DIF > 
%DIF -------
            colorlinks=false,
            urlcolor=blue,
            linkcolor=magenta,
...
\begin{frontmatter}
  ...
  \title{\DIFdelbegin \DIFdel{Long-term }\DIFdelend \DIFaddbegin \DIFadd{Analysis of }\DIFaddend cloud enhancement events \DIFaddbegin \DIFadd{in a 30-year record }\DIFaddend of
global solar irradiance \DIFdelbegin \DIFdel{over }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend Thessaloniki, Greece}
...
Thessaloniki},city={Thessaloniki},postcode={54124},country={Greece},}
  ...
  \begin{abstract}
  \DIFdelbegin \DIFdel{In this study we investigate the characteristics of }\DIFdelend \DIFaddbegin 

  \end{abstract}
    \begin{keyword}
    \DIFadd{cloud enhancement }\sep \DIFadd{total solar radiation }\sep \DIFadd{global horizontal
irradiance }\sep 
    \DIFadd{over irradiance
  }\end{keyword}
...
\addcontentsline{toc}{section}{\DIFadd{Abstract}}

\textbf{\DIFadd{Abstract temporary placed here for technical reasons}}

\DIFadd{We studied the enhancement of the }\DIFaddend global horizontal irradiance \DIFdelbegin \DIFdel{enhancement events }\DIFdelend induced
by clouds over Thessaloniki for the period \DIFdelbegin \DIFdel{1993 }\DIFdelend \DIFaddbegin \DIFadd{1994 }\DIFaddend -- 2023 using data
recorded every one minute. We identified the cloud enhancement \DIFaddbegin \DIFadd{events
}\DIFaddend (CE) \DIFdelbegin \DIFdel{events }\DIFdelend by creating an appropriate \DIFdelbegin \DIFdel{clear-sky }\DIFdelend \DIFaddbegin \DIFadd{clear sky }\DIFaddend irradiance reference with the
...
a trend in CE events of \DIFdelbegin \DIFdel{%
\mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{\(116.9\pm 67.8\,\text{kJ}/\text{year}\)}\DIFdelend \DIFaddbegin \DIFadd{\(+226.4\pm 62.6\,\text{kJ}/\text{year}\)}\DIFaddend . The peak of the CE events was
observed during May and June. \DIFdelbegin \DIFdel{The
  }\DIFdelend \DIFaddbegin \DIFadd{An }\DIFaddend analysis of the total duration of CE
events\DIFaddbegin \DIFadd{, }\DIFaddend showed that durations longer than 5 minutes are very rare, with
...
atmosphere \DIFaddbegin \DIFadd{for the same solar zenith angle }\DIFaddend of up to
\(400\,\text{W}/\text{m}^{2}\), with the \(75\,\%\) of the cases below
\(200\,\text{W}/\text{m}^{2}\). The most active \DIFdelbegin \DIFdel{period }\DIFdelend \DIFaddbegin \DIFadd{season }\DIFaddend of these extreme
events is spring \DIFdelbegin \DIFdel{-- early summer with a secondary peak in
  autumn.
}%DIFDELCMD < \end{abstract}
%DIFDELCMD <     \begin{keyword}
%DIFDELCMD <     %%%
\DIFdel{cloud enhancement }%DIFDELCMD < \sep %%%
\DIFdel{total solar radiation }%DIFDELCMD < \sep %%%
\DIFdel{global horizontal
irradiance }%DIFDELCMD < \sep 
%DIFDELCMD <     %%%
\DIFdel{over irradiance
  }%DIFDELCMD < \end{keyword}
%DIFDELCMD <   

%DIFDELCMD <  \end{frontmatter}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{until early summer and autumn.
}\DIFaddend 
...
equipment. \DIFdelbegin \DIFdel{Significant portion of }\DIFdelend \DIFaddbegin \DIFadd{A significant portion of the relevant }\DIFaddend research has been
...
irradiance \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[ and references
therein]{Cordero2023, Vamvakas2020, CastillejoCuberos2020, Vamvakas2020}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin {[}\DIFadd{\mbox{%DIFAUXCMD
\citet{Cordero2023}}\hskip0pt%DIFAUXCMD
; \mbox{%DIFAUXCMD
\citet{Vamvakas2020}}\hskip0pt%DIFAUXCMD
;
\mbox{%DIFAUXCMD
\citet{CastillejoCuberos2020}}\hskip0pt%DIFAUXCMD
; and references therein}{]}\DIFaddend .

Some of the proposed underling mechanisms of those enhancements \DIFdelbegin \DIFdel{, }\DIFdelend have
...
\DIFdelbegin \DIFdel{enhancement }\DIFdelend \DIFaddbegin \DIFadd{the appearance of CE }\DIFaddend events depend on different interactive factors,
...
there is no evidence \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend whether this trend has affected also the number
...
In this study, we evaluate the effects \DIFdelbegin \DIFdel{CE }\DIFdelend \DIFaddbegin \DIFadd{of CE events }\DIFaddend on GHI by
investigating their frequency of occurrence, intensity, and duration in
a thirty-year \DIFdelbegin \DIFdel{period
}\DIFdelend \DIFaddbegin \DIFadd{record }\DIFaddend of GHI observations at Thessaloniki, Greece. We
...
knowledge there are no other studies that provide trends from such \DIFaddbegin \DIFadd{a
}\DIFaddend long dataset.
...
instrument's sensitivity between subsequent calibrations. \DIFaddbegin \DIFadd{We note that
...
screening was performed, \DIFdelbegin \DIFdel{in order }\DIFdelend to remove inconsistent and erroneous recordings
...
sky camera operates since 2012 and stores images in 5 min time steps. \DIFaddbegin \DIFadd{An
overview of the data analyzed for the study of the CE and ECE events is
given in Figure}\nobreakspace{}\DIFadd{\ref{fig:CLB-daily}, as the daily mean of
the measured GHI.
}
...
\caption{\DIFaddFL{Daily mean values of GHI in Thessaloniki for the period 1994 -- 2023.}}\label{fig:CLB-daily}
\end{figure}
\DIFaddend 
...
level \DIFdelbegin \DIFdel{, }\DIFdelend exceeds the expected value under clear-sky conditions. Similarly,
we define as extreme cloud enhancement events (ECE), the cases when GHI
at ground level exceeds the Total Solar Irradiance (TSI) \DIFaddbegin \DIFadd{at the top of
the atmosphere (TOA) for the same SZA}\DIFaddend . Although the duration of these
bursts can vary from seconds to several minutes, here we are constrained
by the temporal resolution of our data to identify \DIFdelbegin \DIFdel{evens }\DIFdelend \DIFaddbegin \DIFadd{events }\DIFaddend with duration
of at minimum one-minute.

For the detection of CE cases\DIFaddbegin \DIFadd{, }\DIFaddend we established a baseline of irradiance
above which we characterized each data point as an enhancement event and
calculated the over irradiance (\DIFdelbegin \DIFdel{OIR}\DIFdelend \DIFaddbegin \DIFadd{OI}\DIFaddend ). The \DIFdelbegin \DIFdel{OIR }\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend is defined as the irradiance
difference of the measured one-minute GHI from the
\(\text{GHI}_\text{ref}\) corresponding to cloud-free atmosphere. First,
we \DIFdelbegin \DIFdel{tested two simple approaches }\DIFdelend \DIFaddbegin \DIFadd{used a simple approach }\DIFaddend for the determination of
...
to our data \citep{Natsis2023}. We \DIFdelbegin \DIFdel{have tested both methods }\DIFdelend \DIFaddbegin \DIFadd{created a threshold }\DIFaddend by using an
appropriate relative \DIFdelbegin \DIFdel{threshold }\DIFdelend and/or an additional constant offset. The initial
...
vapor which were not \DIFdelbegin \DIFdel{taken into account in the two simple methods}\DIFdelend \DIFaddbegin \DIFadd{considered in the simple method}\DIFaddend . To produce a more
...
variables \DIFdelbegin \DIFdel{in order }\DIFdelend to create a look-up table (LUT) for the estimation of the
...
\DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend Cimel sun photometer \DIFdelbegin \DIFdel{that belongs to }\DIFdelend \DIFaddbegin \DIFadd{which is part of }\DIFaddend the Aerosol Robotic Network
...
study was made using simulations with the RTM \DIFdelbegin \DIFdel{based on }\DIFdelend \DIFaddbegin \DIFadd{to account for }\DIFaddend the
long-term variations of the AOD. As AERONET data start only in 2003, we
used for the period \DIFdelbegin \DIFdel{1993 }\DIFdelend \DIFaddbegin \DIFadd{1994 }\DIFaddend -- 2005 estimates of changes in AOD at
...
AOD at \(340\,\text{nm}\) is \DIFdelbegin \DIFdel{\(0.403\) with a trend }\DIFdelend \DIFaddbegin \DIFadd{\(0.4\) with a change }\DIFaddend of
...
\(\text{GHI}_\text{ref}\), \DIFdelbegin \DIFdel{in order }\DIFdelend to create a more realistic representation of
the clear-sky irradiance for the whole period of study. \DIFaddbegin \DIFadd{The rate of
...
\caption{Simulated long-term change in clear sky irradiance relative to the climatological values due to changes in AOD in Thessaloniki for the period \DIFdelbeginFL \DIFdelFL{1993 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1994 }\DIFaddendFL -- 2023.}\label{fig:CS-change}
\end{figure}
...
\DIFdelbegin \DIFdel{In this study our main focus }\DIFdelend \DIFaddbegin \DIFadd{One of the goals of this study }\DIFaddend was to quantify the \DIFdelbegin \DIFdel{over irradiance }\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend related to CEs. A
...
\DIFdelbegin \DIFdel{in order }\DIFdelend to strengthen the robustness of the methodology and to compensate for
...
measured intensity of the irradiance exceeds the TSI \DIFdelbegin \DIFdel{at the top of the
atmosphere}\DIFdelend \DIFaddbegin \DIFadd{on horizontal plane
at TOA}\DIFaddend . In this case the threshold \(E\) is given by: \begin{equation}
\text{ECE}: E > \cos(\DIFdelbegin \DIFdel{\theta}\DIFdelend \DIFaddbegin \DIFadd{\text{SZA}}\DIFaddend ) \cdot E_{\odot} \frac{r^2_\text{m}}{r^2}
\label{eq:ECE}
\end{equation} where: \DIFdelbegin \DIFdel{\(\theta\) }\DIFdelend \DIFaddbegin \DIFadd{\(\text{SZA}\) }\DIFaddend the solar zenith angle,
\(E_{\odot}\) \DIFaddbegin \DIFadd{is }\DIFaddend the solar constant \DIFdelbegin \DIFdel{adjusted for the actual Sun -- Earth distance
}\DIFdelend \DIFaddbegin \DIFadd{at the mean Sun - Earth distance
(\(1361\,\text{W}/\text{m}^2\)), }\DIFaddend \(r\) \DIFaddbegin \DIFadd{is the actual Sun - Earth
distance }\DIFaddend and \(r_\text{m}\) is the mean Sun -- Earth distance of
...
\caption{Example of CE identification in Thessaloniki for 2019-07-11. The green line with blue symbols depicts the measured GHI in one minute steps. \DIFdelbeginFL \DIFdelFL{The red }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Red }\DIFaddendFL line shows the modelled threshold $E$ for the detection of CE events, which are denoted with red circles.\DIFdelbeginFL \DIFdelFL{The black }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Black }\DIFaddendFL curve represents the \DIFaddbeginFL \DIFaddFL{TOA }\DIFaddendFL TSI \DIFdelbeginFL \DIFdelFL{at the top of the atmosphere, adjusted for the actual Sun-Earth distance and multiplied by the cosine of SZA}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{on horizontal plane}\DIFaddendFL .}\label{fig:example-day}
\end{figure}

\begin{figure}[H]

{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{../images/P-example-years-13} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{../images/P-example-years-12} 
\DIFaddendFL 

}

\caption{Example scatter plot of the measured GHI and the reference clear sky irradiance in Thessaloniki for the year 2005. The over-irradiance for CE and ECE events is color coded, while the remaining data points are shown in black. \DIFaddbeginFL \DIFaddFL{The reference green line goes through the origin with a slope of one.}\DIFaddendFL }\label{fig:example-year}
\end{figure}
...
entire dataset (\DIFdelbegin \DIFdel{\(6.1\) }\DIFdelend \DIFaddbegin \DIFadd{\(6\) }\DIFaddend million of one-minute GHI measurements),
\DIFdelbegin \DIFdel{\(1.76\,\%\) }\DIFdelend \DIFaddbegin \DIFadd{\(1.68\,\%\) }\DIFaddend were identified as CE events and \DIFdelbegin \DIFdel{\(0.036\,\%\) }\DIFdelend \DIFaddbegin \DIFadd{\(0.04\,\%\) }\DIFaddend as ECE
events. The highest recorded GHI due to CE was
\(1416.6\,\text{W}/\text{m}^2\) on 24~May 2007 \DIFdelbegin \DIFdel{, corresponding to OIR of \(345.9\,\text{W}/\text{m}^2\)}\DIFdelend \DIFaddbegin \DIFadd{at a SZA of
\(19.9^\circ\) corresponding to OI of \(338.4\,\text{W}/\text{m}^2\) or
\(31.4\,\%\) above the threshold}\DIFaddend . The strongest CE event of \DIFdelbegin \DIFdel{\(49.7\,\%\)
}\DIFdelend \DIFaddbegin \DIFadd{\(49.9\,\%\)
}\DIFaddend above the clear sky threshold was observed on 28~October \DIFdelbegin \DIFdel{2016. }\DIFdelend \DIFaddbegin \DIFadd{2016 at a SZA
of \(59.2^\circ\) with a GHI value of \(861.8\,\text{W}/\text{m}^2\) and
a OI of \(286.9\,\text{W}/\text{m}^2\). Both cases are ECE events with
\(136.8\,\text{W}/\text{m}^2\) and \(164.5\,\text{W}/\text{m}^2\) above
the TSI at TOA for the same SZA, respectively. }\DIFaddend In the following sections
we are discussing the long-term trends and variability of the CE events
as well as of the corresponding \DIFdelbegin \DIFdel{OIR }\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend and excess irradiation.
...
events over Thessaloniki, as well as to the average \DIFdelbegin \DIFdel{OIR }\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend and excess
...
mean \DIFdelbegin \DIFdel{OIR }\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend and the yearly excess irradiation for the period \DIFdelbegin \DIFdel{1993 }\DIFdelend \DIFaddbegin \DIFadd{1994 }\DIFaddend -- 2023,
...
}\DIFaddend ) and the excess irradiation
(\DIFdelbegin \DIFdel{\(116.9 \pm 67.8\,\text{kJ}/\text{year}\)}\DIFdelend \DIFaddbegin \DIFadd{\(+226.4 \pm 62.6\,\text{kJ}/\text{year}\)}\DIFaddend ), which are also
statistically significant. In contrast the trend of the yearly mean \DIFdelbegin \DIFdel{OIR
}\DIFdelend \DIFaddbegin \DIFadd{OI
}\DIFaddend is negligible (\DIFdelbegin \DIFdel{\(0.1\pm 0.1\,\text{W}/\text{m}^2/\text{year}\)}\DIFdelend \DIFaddbegin \DIFadd{\(+0.12\pm 0.1\,\text{W}/\text{m}^2/\text{year}\)}\DIFaddend ) and of
no statistical significance. The average \DIFdelbegin \DIFdel{OIR }\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend for the entire period is
\DIFdelbegin \DIFdel{\(39.9\pm 2.7\,\text{W}/\text{m}^2\)}\DIFdelend \DIFaddbegin \DIFadd{\(+39.7\pm 2.8\,\text{W}/\text{m}^2\)}\DIFaddend . The interannual variability of
...
irradiance values, which \DIFdelbegin \DIFdel{can }\DIFdelend exceed the equivalent clear sky irradiance by a
significant amount.
...
\begin{figure}% [h!]
    ...
    \caption{Time series for the period \DIFdelbeginFL \DIFdelFL{1993 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1994 }\DIFaddendFL -- 2023 of (a) the yearly CE number of occurrences, (b) the yearly mean \DIFdelbeginFL \DIFdelFL{OIR }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{OI }\DIFaddendFL and (c) the yearly excess irradiation. The black lines represent the linear trends on the yearly data.}\label{fig:P-energy}
\end{figure}
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{../images/clim-CE-month-norm-MAX-median-N-MW-1} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{../images/P-CE-climatology-normlz-1} 
\DIFaddendFL 

}

\caption{Seasonal variability of the number of CE events \DIFaddbeginFL \DIFaddFL{relative to the corresponding number of GHI measurements }\DIFaddendFL in Thessaloniki for the period \DIFdelbeginFL \DIFdelFL{1993 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1994 }\DIFaddendFL -- 2023\DIFdelbeginFL \DIFdelFL{normalized to the maximum occurring in June}\DIFdelendFL , in the form of a box and whisker plot. The \DIFdelbeginFL \DIFdelFL{monthly values have also been normalized to the relative abundance of valid GHI observations. The }\DIFdelendFL box contains the data between the lower $25\,\%$ and the upper $75\,\%$ percentiles, the thick horizontal line and the diamond symbol represent the median and the mean values, respectively. The vertical lines (whiskers) extend between the maximum and minimum monthly values.}\label{fig:relative-month-occurrences}
\end{figure}

The distribution of the number of CE events as a function of \DIFdelbegin \DIFdel{OIR }\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend is shown
in Figure\nobreakspace{}\ref{fig:ovir-distribution}. \DIFdelbegin \DIFdel{Apparently,
there }\DIFdelend \DIFaddbegin \DIFadd{There }\DIFaddend is an inverse
relationship between the frequency of CE events and \DIFdelbegin \DIFdel{OIR }\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend with an
...
events. For the majority (over \(62\,\%\)) of the CE events the \DIFdelbegin \DIFdel{OIR
}\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend is
below the long-term average of \DIFdelbegin \DIFdel{\(39.9\,\text{W}/\text{m}^2\)}\DIFdelend \DIFaddbegin \DIFadd{\(39.7\,\text{W}/\text{m}^2\)}\DIFaddend , while
about \(8.1\,\%\) of the events correspond to \DIFdelbegin \DIFdel{OIR }\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend larger than
...
Thessaloniki. Similar distribution of CE events, albeit with larger \DIFdelbegin \DIFdel{OIR
}\DIFdelend \DIFaddbegin \DIFadd{OI
}\DIFaddend values, has been reported by \citet{Vamvakas2020}, for the city of
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{../images/P-relative-distribution-diff-1} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{../images/P-relative-distribution-diff-2} 
\DIFaddendFL 

}

\caption{Relative frequency distribution of CE events in Thessaloniki for the period \DIFdelbeginFL \DIFdelFL{1993 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1994 }\DIFaddendFL -- 2023 as a function of \DIFdelbeginFL \DIFdelFL{OIR}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{OI. The histogram was split in two plots with different y-axis scales for better readability}\DIFaddendFL .}\label{fig:ovir-distribution}
\end{figure}
...
duration in steps of one minute. We have identified \DIFdelbegin \DIFdel{28468 }\DIFdelend \DIFaddbegin \DIFadd{26837 }\DIFaddend groups of CE
...
\caption{Relative frequency distribution of CE groups of consequent CE events according to their duration for Thessaloniki in the period \DIFdelbeginFL \DIFdelFL{1993 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1994 }\DIFaddendFL -- 2023. The histogram was split in two plots with different y-axis scales for better readability.}\label{fig:ceg-duration-distribution}
\end{figure}
...
irradiation (\DIFdelbegin \DIFdel{e.g., }\DIFdelend \(<5\,\text{kJ}/\text{m}^2\)) and small duration
(\DIFdelbegin \DIFdel{e.g., }\DIFdelend \(<5\,\text{min}\))\DIFdelbegin \DIFdel{while }\DIFdelend \DIFaddbegin \DIFadd{. For example the extra energy flux of
\(5\,\text{kJ}/\text{m}^2\) for \(5\,\text{min}\) is equivalent to OI of
\(\sim17\,\text{W}/\text{m}^2\) for the same duration. While }\DIFaddend groups with
...
\caption{Relation of excess irradiation of CE groups with their duration for Thessaloniki in the period \DIFdelbeginFL \DIFdelFL{1993 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1994 }\DIFaddendFL -- 2023. The logarithmic color scale denotes the frequency of the respective groups of events.}\label{fig:group-2d}
\end{figure}
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{../images/clim-ECE-month-norm-MAX-median-N-2} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{../images/P-ECE-climatology-normlz-1} 
\DIFaddendFL 

}

\caption{Seasonal variability of the number of ECE events \DIFaddbeginFL \DIFaddFL{relative to the the corresponding number of GHI measurements }\DIFaddendFL in Thessaloniki for the period \DIFdelbeginFL \DIFdelFL{1993 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1994 }\DIFaddendFL -- 2023\DIFdelbeginFL \DIFdelFL{normalized to the maximum occurring in March}\DIFdelendFL , in the form of a box and whisker plot. The box contains the data between the lower $25\,\%$ and the upper $75\,\%$ percentiles. The thick horizontal line and the diamond symbol represent the median and the mean values, respectively. The vertical lines (whiskers) extend between the maximum and minimum monthly values.}\label{fig:relative-month-occurancies-ECE}
\end{figure}

The distribution of the ECE events
(Figure\nobreakspace{}\ref{fig:P-extreme-distribution}) shows that in
rare cases the \DIFdelbegin \DIFdel{OIR }\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend can exceed the TSI \DIFaddbegin \DIFadd{at TOA for the same SZA }\DIFaddend even by
more than \(400\,\text{W}/\text{m}^2\), while in \(75\,\%\) of the cases
the \DIFdelbegin \DIFdel{OIR
}\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend is below \(200\,\text{W}/\text{m}^2\). The \DIFdelbegin \DIFdel{OIR }\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend for the most
frequent ECEs ranges between \(140\) and \(180\,\text{W}/\text{m}^2\).
These findings are in accordance with the results of
\citet{Vamvakas2020}, the only difference being that the \DIFdelbegin \DIFdel{OIR }\DIFdelend \DIFaddbegin \DIFadd{OI }\DIFaddend values
reported for are higher than those for Thessaloniki.

\begin{figure}

{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{../images/P-extreme-distribution-1} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{../images/P-extreme-distribution-2} 
\DIFaddendFL 

}

\caption{Distribution of ECE events in Thessaloniki for the period \DIFdelbeginFL \DIFdelFL{1993 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1994 }\DIFaddendFL -- 2023. \DIFaddbeginFL \DIFaddFL{Measured as extra irradiance over TSI at TOA for the same SZA.}\DIFaddendFL }\label{fig:P-extreme-distribution}
\end{figure}
...
identify cases of CE events in Thessaloniki for the period \DIFdelbegin \DIFdel{1993 }\DIFdelend \DIFaddbegin \DIFadd{1994 }\DIFaddend -- 2023.
...
\DIFdelbegin \DIFdel{\(116.9\pm 67.8\,\text{kJ}/\text{year}\)}\DIFdelend \DIFaddbegin \DIFadd{\(+226.4\pm 62.6\,\text{kJ}/\text{year}\)}\DIFaddend . The most active months of CE
