...
duration of CE events, showed that with durations longer than 5 minutes are very rare,
...
knitr::include_graphics("../images/P_daily_trend-3.png")
```
...
$`r round(MC[Typ == "Max Glob", wattGLB],1)`\,\text{W}/\text{m}^2$ on
 `r strftime(MC[Typ == "Max Glob", Date], "%d\\ %B %Y")` at a SZA of
$`r round(MC[Typ == "Max Glob", SZA],1)`^\circ$ corresponding to OI of
$`r round(MC[Typ == "Max Glob", GLB_diff],1)`\,\text{W}/\text{m}^2$ or
$`r round(100*MC[Typ == "Max Glob", GLB_ench],1)`\,\%$ above the threshold.
The strongest CE event of
$`r round(100*MC[Typ == "Max Enha", GLB_ench],1)`\,\%$ above the 
cloud-free threshold was observed on 
`r strftime(MC[Typ == "Max Enha", Date], "%d\\ %B %Y")` at a SZA of
$`r round(MC[Typ == "Max Enha", SZA],1)`^\circ$ with a GHI value of 
$`r round(MC[Typ == "Max Enha", wattGLB],1)`\,\text{W}/\text{m}^2$ and a OI of
$`r round(MC[Typ == "Max Enha", GLB_diff],1)`\,\text{W}/\text{m}^2$.
Both cases are ECE events with
$`r round(MC[Typ == "Max Glob", wattGLB - ETH],1)`\,\text{W}/\text{m}^2$ and
$`r round(MC[Typ == "Max Enha", wattGLB - ETH],1)`\,\text{W}/\text{m}^2$ above the
