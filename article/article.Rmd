...
## trends with "complete data" !!!!!
yearlyCE <- yearlytrends[DATA == "ST_E_yearly_Complete"]

## some values
some01 <- readRDS("~/MANUSCRIPTS/02_enhancement/data/some_values_01.Rds")
...
$`r plus_sign(format(round(yearlyCE[var=="GLB_diff.sum",slope],1),nsmall=1))`\pm `r format(round(2*yearlyCE[var=="GLB_diff.sum",slope.sd],1),nsmall=1)`\,\text{kJ}/\text{year}$.
To our knowledge, such long-term changes in CE events have not been presented in the past.
...
```{r CLB-daily, echo=FALSE, fig.cap="Time-series of daily mean GHI measured at Thessaloniki for the period 1994 -- 2023."}
knitr::include_graphics("../images/P_daily_trend-5.pdf")
```
...
$`r C4_GLB_diff_THRES`\,\text{W}/\text{m}^2$.  The long-term difference between the
...
```{r validation-GHI, echo=FALSE, fig.cap="Scatter plot of the measured GHI and the $\\text{GHI}_\\text{ref}$, for cloud-free days with data availability of more than $80\\,\\%$. The linear regression on the data is also shown on the figure with the blue line.", fig.pos='H'}
knitr::include_graphics("../images/P-validation-cloudfree-GHI-1.pdf")
```
...
```{r example-day, echo=FALSE, fig.cap="Example of CE identification in Thessaloniki for 2019-07-11. The green line with blue symbols depicts the measured GHI in one-minute steps. The red line shows the modeled threshold for the detection of CE events, which are denoted with red circles. The black line represents the TOA TSI on a horizontal plane, equivalent to the threshold for the identification of ECE events, shown with magenta circles. The purple line is the modeled cloud-free irradiance. The dark yellow line is the solar constant of $1367\\,\\text{W}/\\text{m}^{2}$. The three sky images below the figure are taken at three characteristic instances (a, b, c) denoted on the figure with vertical orange dashed lines.", fig.pos='H'}
knitr::include_graphics("../images/P-example-day-3.pdf")
```

```{r example-year, echo=FALSE, fig.cap="Example scatter plot of the measured GHI and the reference cloud-free irradiance in Thessaloniki for the year 2005. The over-irradiance for CE and ECE events is color coded, while the remaining data points are shown in black. The reference green line goes through the origin with a slope of unity.", fig.pos='H'}
knitr::include_graphics("../images/P-example-years-12.pdf")
```
...
$`r round(100*MC[Typ == "Max Enh abs", GLB_ench],1)`\,\%$ above the
cloud-free threshold was observed on
`r strftime(MC[Typ == "Max Enh abs", Date], "%d\\ %B %Y")` at a SZA of
$`r round(MC[Typ == "Max Enh abs", SZA],1)`^\circ$ with a GHI of
...
($`r plus_sign(round(yearlyCE[var == "GLB_diff.mean", slope], 1))`\pm `r round(2 * yearlyCE[var == "GLB_diff.mean", slope.sd], 1)`\,\text{W}/\text{m}^2/\text{year}$)
...
```{r P-energy, echo=FALSE, fig.cap="Time-series of (a) the yearly CE number of occurrences, (b) the yearly mean OI and (c) the yearly excess irradiation at Thessaloniki for the period 1994 – 2023. The black lines represent the linear trends on the yearly data."}
knitr::include_graphics("../images/P-energy-complete-multi-3.pdf")
```
...
```{r relative-month-occurrences, echo=FALSE, fig.cap="Seasonal variability of the relative occurrence of CE events in Thessaloniki for the period 1994 -- 2023, in the form of a box and whisker plot. The box contains the data between the lower $25\\,\\%$ and the upper $75\\,\\%$ percentiles, with the thick horizontal line representing the median. The vertical lines (whiskers) extend between the maximum and minimum monthly values."}
knitr::include_graphics("../images/P-CE-climatology-normlz-1.pdf")
```
...
knitr::include_graphics("../images/clim-CE-month-norm-MAX-median-N-MW-1.pdf")
```
-->
...
For the majority (over $`r round(some01$Enh_pc_below_mean,0)`\,\%$) of
the CE events the OI is below the long-term average of
$`r round(some01$Enh_OI_AVG,1)`\,\text{W}/\text{m}^2$, while
$`r round(some01$Enh_pc_below_100,1)`\,\%$ of the events correspond to OI larger than
$100\,\text{W}/\text{m}^2$ and up to the highest value of
$`r round(some01$Max_OI,1)`\,\text{W}/\text{m}^2$.
...
knitr::include_graphics("../images/P-relative-distribution-diff-2.pdf")
```
...
identified `r ST_G0[, .N]` groups of CE events in the whole period of study, where
...
$`r round(ST_G0[which.max(GLB_diff.N), SZA.min],1)`^\circ$ and
...
$`r round(100*ST_G0[,sum(GLB_ench.N<=5)/.N],0)`\,\%$ have a duration of
...
# knitr::include_graphics("../images/groups-1.pdf")
knitr::include_graphics("../images/groups-7.pdf")
```
...
```{r group-2d, echo=FALSE, fig.cap="Relation of excess irradiation of CE groups with their duration for Thessaloniki in the period 1994 -- 2023. The logarithmic color scale denotes the frequency of the respective groups. Note that 19 points for groups with duration higher than 60 min are not shown."}
knitr::include_graphics("../images/P-groups-bin2d-1.pdf")
```
...
```{r relative-month-occurancies-ECE, echo=FALSE, fig.cap="Seasonal variability of the relative occurrence of ECE events in Thessaloniki for the period 1994 -- 2023, in the form of a box and whisker plot. The box contains the data between the lower $25\\,\\%$ and the upper $75\\,\\%$ percentiles, with the thick horizontal line representing the median. The vertical lines (whiskers) extend between the maximum and minimum monthly values."}
knitr::include_graphics("../images/P-ECE-climatology-normlz-1.pdf")
```
<!--
...
knitr::include_graphics("../images/clim-ECE-month-norm-MAX-median-N-2.pdf")
```
-->
...
```{r P-extreme-distribution, echo=FALSE, fig.cap="Relative frequency distribution of ECE events, in Thessaloniki for the period 1994 -- 2023, as a function of the extra irradiance over TSI at TOA for the same SZA."}
knitr::include_graphics("../images/P-extreme-distribution-2.pdf")
```
