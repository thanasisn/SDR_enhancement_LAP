...
\text{CE} : \text{GHI}_\text{i} > \text{GHI}_\text{CSlim,i}, \text{where} \begin{cases}
 \text{GHI}_\text{CSlim,i} = `r C4_cs_ref_ratio` \cdot \text{GHI}_\text{CSm,i}, & \text{$\theta \leq `r C4_lowcut_sza`^\circ$}\\
\text{GHI}_\text{CSlim,i} = \left ({ `r C4_lowcut_ratio` + \frac{`r C4_cs_ref_ratio` - `r C4_lowcut_ratio`}{`r C4_lowcut_sza` - `r 90 - BIO_ELEVA`} \cdot (\theta- `r 90 - BIO_ELEVA`) } \right ) \cdot \text{GHI}_\text{CSm,i}, & \text{$ `r 90 - BIO_ELEVA`^\circ > \theta > `r C4_lowcut_sza`^\circ$}\\
\text{Excluded measurements}, & \theta > `r 90 - BIO_ELEVA`^\circ
\end{cases}\label{eq:CE4}
\end{equation}
where: $\theta$ is the solar zenith angle, $\text{GHI}_\text{i}$ the measured
irradiance, and $\text{GHI}_\text{CSm}$ the selected modeled clear sky irradiance, and $i$ each of the one-minute observation.
