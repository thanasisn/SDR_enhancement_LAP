...
To be able to detect the CE cases, we had establish a baseline, above which we can
characterize each data point as enhancement event, by estimating the occurring over
irradiance.  To have an estimation and a first insight on the phenomenon, we
experimented with two simpler approaches as the reference.  The Haurwitz's model
[@Haurwitz1945], which is as simple clear sky we have previous adjusted and had good
fit with our data [@Natsis2023], and the TSI at the top of the atmosphere.  For both
cases we tested using an appropriate relative threshold and/or an constant offset.
The initial results, showed that we can detect a big portion of the CE events. But it
was evident, by inspecting the daily plot of irradiance, that changes on the
atmospheric conditions introduced numerous false positive and false negative results. 


Initially, we try to implement a simpler approach for the clear sky reference,
without the use of a radiation transfer model. We experimented with a 
...
Aerosol Robotic Network (AERONET) [@Giles2019; @Buis1998], as we operate a Cimel
...
Libradtran [@Emde2016], a similar modeling approach, was also used by @Vamvakas2020.
...
(TSI) provided by NOAA [@Coddington2005].  Also, we applied the effect of the Earth
...
## Criteria for cloud enhancement identification
