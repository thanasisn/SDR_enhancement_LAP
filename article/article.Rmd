...
Our dataset after the proccesing consists of

total data points `r ST_total$wattGLB.N`

of which CE are `r ST_total$GLB_diff.N_pos`

and `r ST_extreme_total$GLB_diff.N_pos` are ECE
...
dailytrends[DATA == "ST_E_daily"]


```

By computing the mean daily over irradiance for all the CE case, we were able to dirive a trend on the mean CE intensity of .....
...
```{r CEmeanDaily, echo=FALSE, fig.cap="Trend of the daily excess energy due to CE"}
